import sharedPref, {sharedPrefKeys} from '@app/services/sharedPref';
...
const useLanguageStorage: LanguageDetectorAsyncModule = {
  ...
  detect: async (callback: any) => {
    const lang = sharedPref.getString(sharedPrefKeys.langauge);
    if (lang) return callback(lang);
  },
  init: () => null,
  cacheUserLanguage: async (language: string) => {
    sharedPref.set(sharedPrefKeys.langauge, language);
  },
};
